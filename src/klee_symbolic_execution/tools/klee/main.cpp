...
#include "../../lib/Core/GuideExecutionInfo.h"
...
bool GuidedExecutionTag=false;
extern bool GuidedExecution = false;
//------------
static void parseArguments(int argc, char **argv) {
//---bty----handle argv guided-execution
  int argvindex=0;
  for(int i=1; i<argc; i++)
    if(strcmp(argv[i],"--guided-execution")==0){
      printf("KLEE: guided-symbloic-execution\n");
      argvindex=i;
      GuidedExecutionTag=true;
      GuidedExecution = true;
      break;
    }
  if(GuidedExecutionTag==true){
    argc--;
    for(int i=argvindex; i<argc; i++)
      argv[i]=argv[i+1];
  }
...
//----bty---define data structure to save gudied info
std::vector<GuideProfile> GuideSrcInfoByKlee;
//------------
int main(int argc, char **argv, char **envp) {  
//---bty-----read the guided info
  const char* file_name="./GuideSrc.txt";//txt文件所在的路径
...
//--------- 
  ...
  Interpreter *interpreter = 
    ...
    //printf("main: created Interpreter\n");
  ...
  if (!ReplayOutDir.empty() || !ReplayOutFile.empty()) {
    ...
    for (std::vector<KTest*>::iterator
      ...
      //printf("main: calling runFunctionAsMain\n");
