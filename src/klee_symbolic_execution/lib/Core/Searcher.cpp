...
//---bty----decide whether the checklist contain the linenumber belong to the state  
...
bool ListContain(ExecutionState* state){
	...
	for(unsigned int j=0;j<GuideSrcInfoByKlee.size();j++)
		...
		{
//			std::cout<<"Debug: List Contain"<<std::endl;
			return true;}
		}
	return false;
	
}
//---bty---delete the useless state---------- 
void Searcher::GuideStateExecution(std::set<ExecutionState*> &removedStates,ExecutionState* firstbranch,ExecutionState* secondbranch){
       ...
       int nextFalseLine=0;
       if(firstbranch!=NULL){
         KInstruction *ktrue = (*firstbranch).pc;
         const InstructionInfo &itrue = *ktrue->info;
         //std::cout<<"Debug: "<<itrue.file<<"  "<< itrue.line << " "<< "truestate:"<< firstbranch <<std::endl;
         nextTrueLine=itrue.line;
       }
       if(secondbranch!=NULL){
         KInstruction *kfalse = (*secondbranch).pc;
         const InstructionInfo &ifalse = *kfalse->info;
         //std::cout<<"Debug: "<< ifalse.file<<"  "<<ifalse.line << " "<< "falsestate:"<< secondbranch <<std::endl;
         nextFalseLine=ifalse.line;
       }
	if(firstbranch != NULL && secondbranch != NULL){
		if( ListContain(firstbranch) &&	!ListContain(secondbranch) )
		{
			//std::cout<<"Debug: State map to"<< secondbranch<<"has be removed"<<std::endl; 
			removedStates.insert(secondbranch);
		}
		else if( !ListContain(firstbranch) && ListContain(secondbranch) )
			{
//				std::cout<<"Debug: State map to"<< firstbranch<<"has be removed"<<std::endl;  
				removedStates.insert(firstbranch);}
		/*else if( !ListContain(firstbranch) && !ListContain(secondbranch) )
			if(nextTrueLine != 0 && nextFalseLine != 0)
				{std::cout<<"Debug: Both State has be removed"<<std::endl;  removedStates.insert(firstbranch); removedStates.insert(secondbranch);}*/
	}
}
