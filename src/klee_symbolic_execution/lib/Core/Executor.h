...
public:
  ...
  // lyc modification
  void BFO_Check(MemoryManager * manager, ExecutionState & state, KInstruction *ki);
  ...
  // lyc modification
  std::set<std::string> CheckPoints;         // storing the line info in formate like "Core/Executor.c_12"(srcfile_line)
  void insertCheckPoint(std::string point);
  int loadCheckList();         // load checklist from file "checklist_bufferoverflow", which should
                               // be located in the same directory where the source file lives
  bool isCheckPoint(std::string lineOfCode);   // return true if lineOfCode is within CheckPoints
 
  //gfj add
  std::set<std::string> CheckFiles;         // storing the check file info in formate like "Executor.c"(srcfile)
  //void insertCheckFile(std::string file);
  int loadCheckFiles();         // load checklist from file "checklist_fileNames", which should
                               // be located in the same directory where the source file lives
  bool isCheckFile(std::string fileName);   // return true if fileName is within CheckFiles
  //int _vscprintf(const char * format, va_list pargs);
  //int calculateFormatLen(char * format,...);
};
