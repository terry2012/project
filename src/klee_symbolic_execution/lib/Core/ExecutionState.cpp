...
// lyc modification
int ExecutionState::isSymbolic(MemoryObject * mo) {
	if(mo == NULL)return -1;
	for(std::vector<std::pair<const MemoryObject *, const Array *> >:: iterator it = this->symbolics.begin(), end = this->symbolics.end();
		it != end; it++) {
		if(it->first == mo)return (it - this->symbolics.begin());
	}
	return -1;
}
