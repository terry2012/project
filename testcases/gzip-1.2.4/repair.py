...
class Method:
	...
	def instrumentTemplate(self):
		...
		if self.mode == 'API-REP':
			self.src_code[self.lineNum-1] = self.tab+self.repairMethod(self.splitParameters(line_Str2),self.mode)+';\n'
			#'snprintf('+self.repairMethod(self.splitParameters(line_Str2),self.mode)+');\n'
...
class MethodStrcpy(Method):
	...
	def repairMethod(self,dest_src,mode):
		...
		if self.mode == 'API-REP':
			condition = 'strncpy('+dest+','+src+',sizeof('+dest+'))'
		return condition
...
class MethodStrcat(Method):
	...
	def repairMethod(self,dest_src,mode):
		...
		if self.mode == 'API-REP':
			condition = 'snprintf('+dest+'+strlen('+dest+'),sizeof('+dest+')-strlen('+dest+'),\"%s\",'+src+')'			
		return condition
...
class MethodSprintf(Method):
	...
	def repairMethod(self,dest_src,mode):
		...
		if mode == 'API-REP':
			condition = 'snprintf('+dest+',sizeof('+dest+'),'+formatStr+')'
		return condition
